...
def route_printer(block, row, col, enter, exist, fpath_finder):
    ...
    if (f"{str(row).zfill(2)}_{str(col).zfill(2)}") == enter:
        block[:, :, 0] = 0
        block[:, :, 1] = 255
        block[:, :, 2] = 0
    if (f"{str(row).zfill(2)}_{str(col).zfill(2)}") == exist:
        block[:, :, 0] = 255
        block[:, :, 1] = 0
        block[:, :, 2] = 0
...
def generate_blocks(rows, cols, route=False, enter=None, exist=None, fpath_finder=None):
...
def route_map_printer(start, end):
    ...
    generate_blocks(cols, rows, route=True, enter=start, exist=end, fpath_finder=path_finder)
    contact_image = contact_blocks(rows, cols)
