...
def generate_maze(rows, cols):
    ...
    path_cells = {f"{r:02}_{c:02}" for r, c in visited}
    
    for cell in all_cells:
        if cell not in path_cells and cell != f"{entrance[0]:02}_{entrance[1]:02}" and cell != f"{exit[0]:02}_{exit[1]:02}":
            r, c = map(int, cell.split('_'))
            if random.random() < 0.2:  # 20% 概率添加副路径
                neighbors = []
                if r > 1 and f"{r-1:02}_{c:02}" not in path_cells:
                    neighbors.append((r-1, c))
                if r < rows and f"{r+1:02}_{c:02}" not in path_cells:
                    neighbors.append((r+1, c))
                if c > 1 and f"{r:02}_{c-1:02}" not in path_cells:
                    neighbors.append((r, c-1))
                if c < cols and f"{r:02}_{c+1:02}" not in path_cells:
                    neighbors.append((r, c+1))
                if neighbors:
                    next_cell = random.choice(neighbors)
                    remove_wall(maze, r, c, next_cell[0], next_cell[1])
...
#########################
# 生成20x20的迷宫
maze_data = generate_maze(20, 20)
maze_data['length'] = [20, 20]
route_data = {}

# 保存迷宫数据到JSON文件
with open('./simple_maze/maze_info.json', 'w') as f:
    json.dump(maze_data, f, indent=4)

with open('./simple_maze/route_info.json', 'w') as f:
    json.dump(route_data, f, indent=4)

print("迷宫已生成并保存到 maze.json")
